...
namespace LeoMapV2.Map
{
    public class LeoMap : Panel, ILeoMap
    {
        ...
        private void PictureBoxMouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                if (Math.Abs(_oldX - e.X) <= 10 || Math.Abs(_oldY - e.Y) <= 10)
                {
        ...
        private async void UpdateMapImage()
        {
